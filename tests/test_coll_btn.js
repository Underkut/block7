...
console.log('시나리오 1 — 세 곳 모두 같은 책 실루엣을 쓴다');
{
  ...
  sc.eq('다섯 곳 모두 아래 두 권은 그대로', slabs, 10);
  // ⚠️ 전체에서 evenodd 를 세면 안 된다 — 다른 아이콘도 컷아웃을 쓴다
  //    (v26-0830-16 담아두기 주머니). 책 커버가 시작되는 path 만 센다.
  sc.eq('다섯 곳 모두 컷아웃 방식(evenodd)',
        (SRC.match(/fill-rule="evenodd" d="M3\.8 8\.2 /g)||[]).length, 5);
  // v26-0818-7, HB — 메뉴의 다른 아이콘들에 비해 여백이 커 보인다는 지적으로
  // viewBox 를 도형 실제 bbox 에 바짝 맞춰 크롭했다(디자인·각도는 그대로, 여백만 축소).
  sc.eq('꽉 찬 실루엣으로 그린다',
        (SRC.match(/viewBox="3\.2 2 17\.6 19\.7" fill="currentColor" stroke="currentColor" stroke-width="0\.7" stroke-linejoin="round"/g)||[]).length, 5);
...
console.log('\n시나리오 1-1 — 커버 컷아웃 모양 (십자가 vs 3줄)');
{
  ...
  // 그리고 v26-0906-2 부터 대시보드 윗줄까지 네 곳
  ...
  sc.eq('십자가 컷아웃이 네 곳', crossCount, 4);
