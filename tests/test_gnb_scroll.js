...
console.log('\n시나리오 4 — GNB 높이와 내순서 끌기');
{
  ...
  sc.eq('로고가 놓이는 칸은 42px', /height:calc\(42px \+ var\(--gnb-safe\)\)/.test(c), true);
  sc.eq('예전 값(46·38·40)으로 되돌아가 있지 않다', /height:calc\((46|38|40)px \+/.test(c), false);

  // ── 시계와 로고 사이 여백 (HB 스크린샷의 연두색 자리) ──
  // env(safe-area-inset-top) 은 시계가 실제로 쓰는 높이보다 넉넉하다. 그 여유만
  // 덜어낸다. 크로미움에서 같은 식을 값으로 검증한 결과(v26-0903-7):
  //   inset  0 → 0 / 20 → 20 / 47 → 33.46 / 59 → 42.01
  sc.eq('위 여백은 --gnb-safe 로 계산한다', /--gnb-safe:max\(min\(env\(safe-area-inset-top, 0px\), 20px\), calc\(env\(safe-area-inset-top, 0px\) \* \.712\)\)/.test(c), true);
  ...
  sc.eq('노치 없는 기기(20px)에서는 덜어내지 않는다 — 20 > 20*.712',
        20 > 20 * 0.712, true);
  sc.eq('다이내믹 아일랜드(59px)에서 위 여백은 42px', Math.round(59 * 0.712), 42);
  ...
  sc.eq('합계는 84px (42 + 42)', Math.round(59 * 0.712) + 42, 84);
  sc.eq('로고(15px)는 칸 가운데 → 아래 여백 13.5px', (42 - 15) / 2, 13.5);
