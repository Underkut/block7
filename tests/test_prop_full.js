...
console.log('\n시나리오 17 — 타일뷰의 갈래 탭과 명제 표시 (v26-0831-19, HB)');
{
  ...
  sc.eq('같은 탭 표를 쓴다(칩 하나)', SRC_DEV.includes("row.innerHTML=_vpModeChipHTML(_vgTab(),\"vgCycleTab()\");"), true);
  sc.eq('칩을 누르면 말씀→명제→함께로 돈다',
        SRC_DEV.includes('function vgCycleTab(){ vgSetTab(_vpNextTab(_vgTab())); }'), true);
  sc.eq('거르는 자리는 한 곳',
