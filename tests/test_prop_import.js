...
// ═══ 3-B-2. ⚠️⚠️ '인용 본문'은 성경 소속에 **절대** 쓰지 않는다 ═══
console.log('시나리오 3-B-2 — 인용 본문은 성경 필터에 안 쓴다');
{
  // HB 신고 (26-0831) — "마태복음 필터링인데 왜 삼상이 나오고, 히브리서
  //   필터링인데 마가복음이 나오냐. '설교 본문'에는 없는데 '인용 본문'에서
  //   가져온 성경까지 필터링에 노출되고 있는 느낌이다. 그거 쓰지 마라."
  const H2=['명제 ID','날짜','카테고리','설교 제목','설교 본문','성경권','인용 본문',
            '명제','대표 문구','주제 태그','데이터 상태'];
  const put=(v)=>{const r=new Array(H2.length).fill('');
    Object.keys(v).forEach(k=>r[H2.indexOf(k)]=v[k]);return r;};
  // ① 시트의 '성경권' 열이 인용 본문까지 헤아려 적혀 있어도 따라가지 않는다
  const r1=put({'명제 ID':'P0311','명제':'전도서 설교의 명제','데이터 상태':'활성',
    '설교 본문':'전도서 3:1',
    '성경권':'전도서, 마태복음, 사무엘상, 마가복음',
    '인용 본문':'전 3:1; 마 5:13; 삼상 1:1; 막 1:1'});
  const it1=_propRowsToItems([H2,r1])[0];
  sc.eq('설교 본문의 성경만 걸린다',it1.books,['전도서']);
  sc.eq('마태복음 필터에 안 걸린다',it1.books.includes('마태복음'),false);
  sc.eq('사무엘상 필터에 안 걸린다',it1.books.includes('사무엘상'),false);
  // ② 설교 본문이 비면 **성경 없는 명제**다 — 인용 본문으로 물러서지 않는다
  const r2=put({'명제 ID':'P0312','명제':'설교 본문이 빈 명제','데이터 상태':'활성',
    '성경권':'마태복음','인용 본문':'마 5:13; 막 1:1'});
  const it2=_propRowsToItems([H2,r2])[0];
  sc.eq('인용 본문을 장절로 쓰지 않는다',it2.refs,[]);
  sc.eq('소속 성경도 없다',it2.books,[]);
}
