...
//    리스너로 스냅샷이 오면 쓰기가 열리고 곧바로 병합에 들어가는데, base 가 없어
//    빈 로컬이 충돌에서 이겼다. 합집합인 로그만 살고 할일·설정·배치가 사라졌다.
// ⚠️⚠️ **버전 차이와 무관하다.** HB 사고 때 모든 기기가 같은 버전(0813-1)이었다.
//    구버전 경로(_fbVerIsOlder)를 타지 않아도, base 만 없으면 리스너의 평범한
//    '미전송 편집 병합' 과 _fbCommit 의 트랜잭션 병합에서 똑같이 일어난다.
//    그래서 방어를 구버전 경로가 아니라 **_fbMergeGuarded** 에 두고 세 곳에 다 걸었다.
