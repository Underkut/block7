...
{
  // ⚠️ 태그가 여럿일 때 예전에는 그냥 랜덤이었다. 시트 '강조 문구' 열이 짚어 준
  //    말과 그림이 따로 놀지 않도록 후보 묶음마다 점수를 매겨 고른다.
  const many = (v, n) => {
    const s = new Set();
    for (let i = 0; i < (n || 60); i++) { const p = box.pick(v); s.add(p && p.id); }
    return [...s].sort();
  };
  const V2 = (text, tags, hi) => ({ ref: '테스트 1:1', krText: text, tags, hi });

  sc.eq('강조가 짚은 태그의 그림으로 굳는다',
    many(V2('여호와를 신뢰하고 그의 명령에 순종하라', ['평안', '순종'], '순종하라')), ['m-bow']);
  sc.eq('강조가 다른 쪽이면 그쪽으로 굳는다',
    many(V2('여호와를 신뢰하고 그의 명령에 순종하라', ['평안', '신뢰'], '신뢰하고')), ['m-lean']);
  sc.eq('강조도 본문도 안 걸리면 예전처럼 둘 다 나온다',
    many(V2('여호와께서 그를 붙드심이로다', ['평안', '순종'], ''), 300).length > 1, true);
  // 묶음의 다른 별칭으로도 이어진다 — 태그는 '순종'인데 강조에는 '청종'이라 적힌 경우
  sc.eq('같은 묶음의 다른 별칭도 본다',
    many(V2('내 목소리를 청종하라', ['평안', '순종'], '청종하라')), ['m-bow']);
  // ⚠️ 한 글자 태그는 파묻힌 자리에 안 걸려야 한다 ('회복'의 '복', '형상'의 '상')
  sc.eq("'회복'의 '복' 에 공급 태그가 걸리지 않는다",
    many(V2('내 영혼을 회복시키시고', ['빛', '공급'], '회복시키시고'), 300).length > 1, true);
  sc.eq('강조가 못 걸리면 본문에서 찾는다',
    many(V2('여호와께 순종하는 것이 제사보다 낫고', ['평안', '순종'], '제사보다 낫고')), ['m-bow']);
}
