...
console.log('\n시나리오 13 — 글자용 강조색');
{
  ...
  sc.eq('보조색은 밝기만 민다 — 색조·채도는 그대로',
        /const cand=_thLabRgb\(\[Math\.max\(2,Math\.min\(98,L\[0\]\+dir\*d\)\),L\[1\],L\[2\]\]\)/.test(SRC), true);
  {
    const olive = _themeTokens(themeById('14'), 'light');
    ...
    // HB 가 고른 답 — 채도를 민 초록이 아니라 **팔레트에 있는 브라운**이다.
    // 팔레트 원색 #bc6c25 를 읽히는 만큼만 어둡게 민 값 (v26-0903-4).
    sc.eq('14 라이트 보조색은 브라운', olive['--sec'], '#bc6c25');
    sc.eq('14 라이트 글자용 강조색은 그 브라운', olive['--ac-tx'], '#ad6018');
  }
