...
console.log('\n시나리오 3-1 — 닫기와 크기·높이·색·투명도를 통일한다');
{
  ...
  //    위에서부터 홈·순환/셔플(12) → 말씀 모음 설정(50) → 대시보드(88, v26-0907-8
  //    새로 낀 자리) → 저장 폴더 책갈피(126), 그리고 책갈피가 여는 목록이
  //    그 아래(160)에서 펼쳐진다.
  ...
  sc.eq('대시보드는 그 아래(v26-0907-8)', topAt('.vf-dashbtn', 88), true);
  sc.eq('저장 폴더 버튼은 그 아래', topAt('.vf-keepmenu', 126), true);
  sc.eq('저장 폴더 목록은 다시 그 아래에서 펼쳐진다',
        SRC.includes('.vf-keep-switch{position:absolute;z-index:13;top:calc(env(safe-area-inset-top,0px) + 160px);left:14px;'), true);
