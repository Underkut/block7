...
기준 버전 **v. 26-0823-2** · 전체 26,045줄 · 구역 211개 · 함수 1254개
...
| 5,321~26,042 | 20,722줄 (80%) | JS | 동작 (자바스크립트) |
...
| 25,267 | 앱 버전 비교 ("v. YY-MMDD-N") | `_verNums`, `_verCmp`, `_fbVerIsOlder`, `_mgWhole`, `_mgContainerKeys`, `_mgCountBag`, `_mgEntryArray`, `_mgLogFlat`, `_mgLogNested`, `_mgDay`, `_mgById`, `_fbHasAdoptedCloud`, `_fbCountArrays`, `_fbCountItems` … 외 11개 |
| 25,611 | 데이터 복구: 로컬(localStorage) ↔ 클라우드(Firestore) 비교 | `_dayHasContent`, `_recoverySummary`, `inspectRecoveryDate`, `checkDataRecovery`, `cleanupEmptyDays`, `fbForceUploadLocal` |
| 25,750 | 자동 백업 보기·복원 (동기화 충돌 병합 시 남는 3슬롯) | `showAutoBackups`, `restoreAutoBackup`, `applyRemoteState`, `_fbWarnLegacyWriter`, `_fbHealFromLegacy`, `first`, `_fbMaybeSelfUpdate`, `fbStartListening` |
| 26,024 | DEV MODE BOOTSTRAP | `fbPushState`, `authSignOut`, `checkDataRecovery`, `fbForceUploadLocal`, `showAutoBackups`, `restoreAutoBackup` |
...
`_fbCommit`  `_fbCountArrays`  `_fbCountItems`  `_fbDeviceIdle`  `_fbEnsureSync`  `_fbForceWrite`
`_fbHasAdoptedCloud`  `_fbHealFromLegacy`  `_fbIsUserEdit`  `_fbLoadPersistedBase`  `_fbMaybeSelfUpdate`  `_fbMerge`
`_fbMergeGuarded`  `_fbReady`  `_fbScheduleRetry`  `_fbSetBase`  `_fbVerIsOlder`  `_fbWarnLegacyWriter`
...
`_hiWave`  `_hiWavePoly`  `_hiWob`  `_idbDel`  `_idbGet`  `_idbOpen`
`_idbSet`  `_importVerseRows`  `_initEdgeBack`  `_initHdrDateLongPress`  `_initSettingsSwipe`  `_initSneakMaxWPicker`
`_initVerseAlarmPicker`  `_initVerseBarSwipe`  `_initVerseFullGestures`  `_initVerseGridGestures`  `_initVerseNotifBridge`  `_initVerseSettingsSwipe`
`_initVfCatSheet`  `_invalidateVerseCaches`  `_isDevAccount`  `_isPhoneForm`  `_isTouchDevice`  `_lay`
...
`_mgLogNested`  `_mgWhole`  `_moveDateToastMsg`  `_mviewEventCountsHTML`  `_mviewRowHTML`  `_noiseTile`
`_notifLog`  `_notifLogLSPush`  `_notifLogLSRead`  `_notifLogRead`  `_notifSameRef`  `_notifShowing`
`_notifStep`  `_notifStop`  `_notifyGet`  `_notifySupport`  `_nowHM`  `_numHex`
`_openCellEvent`  `_openCellEventRepaint`  `_openCellTodo`  `_openGhostInput`  `_openVerseByRef`  `_openVerseFromLink`
`_parseCsv`  `_parseVDate`  `_populateMorningTimePickers`  `_publishSharedColl`  `_reactWithToast`  `_readPendingVerse`
...
`_syncCarryScopeBtns`  `_syncCondRows`  `_syncDevInboxVisibility`  `_syncEventDateUI`  `_syncHdrDateOptical`  `_syncHiOverlapRow`
...
`_syncVpTimeField`  `_syncVpTimeList`  `_t2m`  `_thContrast`  `_themePreviewHTML`  `_themeSummaryText`
...
`_vfApplyParallelRule`  `_vfBgCss`  `_vfBottomEl`  `_vfBreakClass`  `_vfCanBreakAt`  `_vfClauseStart`
...
`_vfPairKeep`  `_vfParallelRuns`  `_vfPatternKey`  `_vfPatternPool`  `_vfRenderCard`  `_vfRollVariant`
...
`_vpDiagCopyFallback`  `_vpDiagFmt`  `_vpEveryLabel`  `_vpSave`  `_vpToMin`  `_vpTurnOn`
`_vsetGoColl`  `_vsetRestoreBack`  `_vstabList`  `_vwSize`  `_withFullscreenLayout`  `_withTimeout`
...
`authSetLoading`  `authSignOut`  `authSubmit`  `authToggleMode`  `autoSizeInput`  `axisLabel`
...
`ceToggleGoogleAuto`  `chap`  `checkDataRecovery`  `checkVerseAlarm`  `chM`  `clamp`
`cleanupEmptyDays`  `clearActive`  `clearContactForm`  `clearDropIndicators`  `clearPaint`  `clearTrash`
...
`itemKey`  `K`  `keep`  `kindLabel`  `laySetBp`  `laySetWeekly`
...
`map`  `measure`  `mergeDuplicateVerses`  `mk`  `mkBtn`  `mkDate`
