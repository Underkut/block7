// BLOCK7 푸시 알림 서비스워커 — **대기본** (v. 26-0828-4)
//
// ⚠️⚠️ 이 파일은 **아직 아무 데서도 등록하지 않는다.** 서비스워커는 개발본이
//    따로 없어서(개발본은 Firebase 가 꺼져 있다) 운영에 올리는 것 말고는
//    HB가 아이폰에서 확인할 방법이 없다. 그래서 다 만들어 두고 여기서 기다린다.
//    HB가 "운영본 배포해" 라고 하면 이 파일을 그대로
//    `firebase-messaging-sw.js` 에 덮어쓰는 것이 배포다 (그때 이 머리말은 지운다).
//    검증은 `tests/test_notif_sw.js` 가 이 파일을 직접 읽어서 한다.
//
// ══════════════════════════════════════════════════════════════
